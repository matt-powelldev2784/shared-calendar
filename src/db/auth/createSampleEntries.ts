...
export const createSampleEntires = async (calendarId: string) => {
  const now = new Date();
  // Get Monday of the current week (weekStartsOn: 1 for Monday)
  ...
  const sampleEntries = [
    ...
    },
    {
      ...
      startDate: setMinutes(setHours(addDays(monday, 1), 10), 30), // Tuesday 10:30
      endDate: setMinutes(setHours(addDays(monday, 1), 11), 30), // Tuesday 11:30
      calendarId,
      pendingRequests: [],
    },
    {
      ...
      startDate: setMinutes(setHours(addDays(monday, 2), 13), 15), // Wednesday 13:15
      endDate: setMinutes(setHours(addDays(monday, 2), 15), 0), // Wednesday 15:00
      calendarId,
      pendingRequests: [],
    },
    {
      ...
      startDate: setMinutes(setHours(addDays(monday, 3), 10), 15), // Thursday 10:15
      endDate: setMinutes(setHours(addDays(monday, 3), 11), 0), // Thursday 11:00
      calendarId,
      pendingRequests: [],
    },
  ];
